...
const createUploadLink = ({
  ...
  return new _apolloLink.ApolloLink(operation => {
    ...
    const {
      options,
      body
    } = (0, _apolloLinkHttpCommon.selectHttpOptionsAndBody)(operation, _apolloLinkHttpCommon.fallbackHttpConfig, linkConfig, contextConfig);
    const files = (0, _extractStreamsFiles.extractFilesOrStreams)(body);
    const payload = (0, _apolloLinkHttpCommon.serializeFetchParameter)(body, 'Payload');
    const promises = [];

    if (files.length) {
      delete options.headers['content-type'];
      if (_extractStreamsFiles.isBrowserOrNative) options.body = new FormData();else if (serverFormData) options.body = new serverFormData();else throw new _extractStreamsFiles.NoFormDataException(`FormData function doesn't exist on this server version. \
...
as an argument in 'createUploadLink' function : '{ serverFormData: FormData }'`);
      ...
      options.body.append('map', JSON.stringify(files.reduce((map, {
        path
      }, index) => {
        map[`${index}`] = [path];
        return map;
      ...
      files.forEach(({
        ...
        if ((0, _extractStreamsFiles.isStream)(file)) options.body.append(index, file);else if (file instanceof Promise) promises.push(new Promise((resolve, reject) => {
            file.then(file => {
              const {
                filename,
    ...
    return new _apolloLink.Observable(observer => {
      const {
        controller,
        signal
      } = (0, _apolloLinkHttpCommon.createSignalIfSupported)();
      if (controller) options.signal = signal;
      Promise.all(promises).then(() => {
        linkFetch(uri, options).then(response => {
          operation.setContext({
            response
          });
          return response;
        }).then((0, _apolloLinkHttpCommon.parseAndCheckHttpResponse)(operation)).then(result => {
