...
const isStream = obj => {
  return obj && typeof obj.pipe === 'function' && typeof obj._read === 'function' && typeof obj._readableState === 'object' && obj.readable !== false;
};

exports.isStream = isStream;

const extractFilesOrStreams = (tree, treePath) => {
  if (isBrowserOrNative) return (0, _extractFiles.default)(tree);else {
    if (treePath === void 0) treePath = '';
    var files = [];

    var recurse = function recurse(node, nodePath) {
      Object.keys(node).forEach(function (key) {
        if (!(0, _extractFiles.isObject)(node[key])) return;
    ...
    if ((0, _extractFiles.isObject)(tree)) recurse(tree, treePath === '' ? treePath : treePath + '.');
    return files;
  }
};
